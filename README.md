<div align="center">

# Carlos Expósito
### Software Architect & Senior Full Stack Developer &bull; Perito Informático Colegiado Nº 03624

<p align="center">
  <a href="https://frodygr.github.io/"><img src="https://img.shields.io/badge/Portfolio-frodygr.github.io-0ea5e9?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Portfolio" /></a>
  <a href="https://www.linkedin.com/in/carlos-exposito-ceballo"><img src="https://img.shields.io/badge/LinkedIn-carlos--exposito--ceballo-0077b5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="https://central.sonatype.com/namespace/io.github.frodygr"><img src="https://img.shields.io/badge/Maven_Central-io.github.frodygr-orange?style=for-the-badge&logo=apache-maven&logoColor=white" alt="Maven Central" /></a>
  <a href="https://ceanalisisdigital.es"><img src="https://img.shields.io/badge/Peritaje_Forense-CE_Análisis_Digital-emerald?style=for-the-badge&logo=shield&logoColor=white" alt="Peritaje Forense" /></a>
</p>

</div>

---

## 👨‍💻 Sobre Mí

Soy **Carlos Expósito**, desarrollador y arquitecto de software con **+8 años de experiencia** profesional diseñando y construyendo plataformas digitales de alta concurrencia, arquitecturas backend resilientes y soluciones en la nube. 

* 🏛️ **Arquitectura & Backend**: Especializado en **Java 21+**, **Spring Boot 3**, **Project Loom (Virtual Threads)**, arquitecturas basadas en eventos (Kafka, RabbitMQ) y microservicios *cloud-native* sobre **AWS** y **GCP**.
* ⚖️ **Peritaje Judicial Informático**: Perito Informático Forense Colegiado (**Nº 03624**, Barcelona). Especialista en análisis de evidencias digitales, cadena de custodia bajo norma **ISO/IEC 27037**, peritaje de ciberdelitos y ratificación judicial.
* 📦 **Ecosistema Open Source**: Autor de **7 librerías y herramientas de alto impacto** en el ecosistema Java empresarial publicadas bajo el namespace `io.github.frodygr`.
* 🎮 **Creatividad & Game Dev**: Apasionado del desarrollo de videojuegos experimentales en [itch.io](https://frodygr.itch.io/).

---

## 🚀 Ecosistema Open Source & Librerías Java

<div align="center">

| Proyecto | Descripción | Enlaces |
|:---|:---|:---:|
| **[ScopeFlow](https://github.com/FrodyGr/scopeflow)** | Propagación automática de contexto de hilos (MDC, Tracing) para Virtual Threads en Java 21+. Integración nativa con Spring Boot, OpenTelemetry y Micrometer. | [![Maven Central](https://img.shields.io/maven-central/v/io.github.frodygr/scopeflow-core?color=blue)](https://central.sonatype.com/artifact/io.github.frodygr/scopeflow-core) <br/> [Wiki](https://github.com/FrodyGr/scopeflow/wiki) |
| **[Fuse](https://github.com/FrodyGr/Fuse)** | Circuit Breaker ultraligero (<20 KB) y zero-dependencies para microservicios Java. Concurrencia *lock-free* basada en CAS (`AtomicReference`) de alto rendimiento. | [![Maven Central](https://img.shields.io/maven-central/v/io.github.frodygr/fuse?color=blue)](https://central.sonatype.com/artifact/io.github.frodygr/fuse) <br/> [Wiki](https://github.com/FrodyGr/Fuse/wiki) |
| **[OpenAPIGuard](https://github.com/FrodyGr/OpenAPIGuard)** | Motor de auditoría de seguridad para contratos OpenAPI/Swagger contra OWASP Top 10. Generación nativa de reportes SARIF para pipelines CI/CD y JUnit 5. | [![Maven Central](https://img.shields.io/maven-central/v/io.github.frodygr/openapiguard-junit5?color=blue)](https://central.sonatype.com/artifact/io.github.frodygr/openapiguard-junit5) <br/> [Wiki](https://github.com/FrodyGr/OpenAPIGuard/wiki) |
| **[AuthForge](https://github.com/FrodyGr/authforge)** | Plataforma empresarial de autenticación y autorización para microservicios: JWT, OAuth2 (Google/GitHub), 2FA TOTP, Rate Limiting y 100% de cobertura de código. | [![Maven Central](https://img.shields.io/maven-central/v/io.github.frodygr/authforge?color=blue)](https://central.sonatype.com/artifact/io.github.frodygr/authforge) <br/> [Wiki](https://github.com/FrodyGr/authforge/wiki) |
| **[AgentGuard](https://github.com/FrodyGr/AgentGuard)** | Firewall de seguridad para aplicaciones de IA en Java (Spring AI, LangChain4j). Enmascaramiento reversible de PII con validación Luhn, detección de Prompt Injections y aspecto `@SecurePrompt`. | [![GitHub](https://img.shields.io/badge/GitHub-AgentGuard-181717?logo=github)](https://github.com/FrodyGr/AgentGuard) <br/> [Wiki](https://github.com/FrodyGr/AgentGuard/wiki) |
| **[EvidenceChain](https://github.com/FrodyGr/EvidenceChain)** | Libro mayor criptográfico inmutable con árboles de Merkle y SHA-256. Diseñado bajo estándares periciales judiciales, con verificación de inclusión $O(\log N)$ y aspecto `@AuditedEvidence`. | [![GitHub](https://img.shields.io/badge/GitHub-EvidenceChain-181717?logo=github)](https://github.com/FrodyGr/EvidenceChain) <br/> [Wiki](https://github.com/FrodyGr/EvidenceChain/wiki) |
| **[LoomDoctor](https://github.com/FrodyGr/LoomDoctor)** | Diagnóstico en tiempo real de Virtual Threads en Java 21+. Detección de Carrier Thread Pinning en bloques sincronizados, monitor de saturación de pools de conexiones y Spring Boot Actuator. | [![GitHub](https://img.shields.io/badge/GitHub-LoomDoctor-181717?logo=github)](https://github.com/FrodyGr/LoomDoctor) <br/> [Wiki](https://github.com/FrodyGr/LoomDoctor/wiki) |

</div>

---

## 🛠️ Stack Tecnológico

```
Backend & Concurrency  ──▶  Java 21+ • Spring Boot 3 • Project Loom • Spring Security • Spring Cloud
Cloud & DevOps         ──▶  AWS (Lambda, SQS, S3, EC2) • GCP • Docker • Kubernetes • GitHub Actions
Databases & Cache      ──▶  PostgreSQL • MySQL • MongoDB • Redis • Elasticsearch
Messaging & Streaming  ──▶  Apache Kafka • RabbitMQ
Ciberseguridad         ──▶  Forense Digital (ISO/IEC 27037) • OWASP • Merkle Trees • OAuth2 / JWT
Frontend               ──▶  TypeScript • JavaScript (ES6+) • HTML5 • CSS3 Moderno
```

---

## 📊 Estadísticas de GitHub

<div align="center">
  <img src="https://github-readme-stats-fast.vercel.app/api?username=FrodyGr&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="GitHub Stats" height="165" />
  <img src="https://github-readme-stats-fast.vercel.app/api/top-langs/?username=FrodyGr&layout=compact&theme=tokyonight&hide_border=true" alt="Top Languages" height="165" />
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=FrodyGr&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
</div>

---

## 📬 Contacto & Despacho

* 🌐 **Portfolio**: [frodygr.github.io](https://frodygr.github.io/)
* 💼 **LinkedIn**: [carlos-exposito-ceballo](https://www.linkedin.com/in/carlos-exposito-ceballo)
* ⚖️ **Despacho Forense**: [CE Análisis Digital](https://ceanalisisdigital.es)
* 📧 **Email**: [carlos.cjec@gmail.com](mailto:carlos.cjec@gmail.com)
